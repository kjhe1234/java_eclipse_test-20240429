...
public class Test02 {
	...
	public static void main(String[] args) {
		...
		System.out.println(d / c); // 실수 대 실수 연산은 소수점이 없는 값도 .0 이 붙어서 답이 실수로 나옴
		
		int e = 10;
		double f = 2.5;
		System.out.println(e + f); // 정수 대 실수 연산은 답이 실수로 나옴
		System.out.println(e / f); // 정수 대 실수 연산은 소수점이 없는 값도 .0 이 붙어서 답이 실수로 나옴
		...
		String str1 = (String)"3" + 10; // 문자열 + 정수를 연산하면 양쪽다 문자열로 변환한 후 합침
		...
		String numStr3 = "true";
		boolean numLogic = Boolean.parseBoolean(numStr3); // 문자열 -> 논리변수로 변환
			
		int num1 = 100;
		//String str2 = (str)100;  // 자바에서는 숫자(정수,실수)를 바로 문자열로 변환 불가!
		
		String str2 = String.valueOf(num1); // 정수 100이 문자열 100으로 변환되어 저장
